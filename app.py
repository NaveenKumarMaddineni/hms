...
def open_customer_details():
    fr=open(r"C:\Users\PHANEENDRA S\Desktop\tcs_proj\json\customer_details.json",'r')
...
def home1():
    if request.method == 'POST':
        f=open(r"C:\Users\PHANEENDRA S\Desktop\tcs_proj\json\users.json",'r')
...
def store_customer_details():
    if request.method=='POST':
        ...
        if r['cssnid'] in jr.keys():
            ...
            fw=open(r"C:\Users\PHANEENDRA S\Desktop\tcs_proj\json\customer_details.json",'w')
            jr[r['cssnid']]={"customername":r['customername'],"age":r['age'],'address1':r['address1'],'address2':r['address2'],'city':r['city'],'state':r['state'],'phno':r['phno'],'status':'Active','lastupdated':time.strftime("%D  %H:%M:%S"),'account':{'accountid':"",'amount':0,'accounttype':"","statement":{}}}
...
def store_account():
    ...
    if r['cssnid'] in jr.keys():
        ...
        if d["account"]["accountid"]!="":
            ...
            d['account']["statement"][time.strftime("%d/%m/%Y %H:%M:%S")]={"operation":"Deposited","opamount":r['amount'],'remainingamount':r['amount']}
            jr[r['cssnid']]=d
            fw=open(r"C:\Users\PHANEENDRA S\Desktop\tcs_proj\json\customer_details.json",'w')
            json.dump(jr,fw,indent=2)
            fw.close()
    ...
    if r['cssnid'] in jr.keys():
        if r['accountid']==jr[r['cssnid']]['account']['accountid']:
            ...
            if rm>=w and rm-w>=0:
                ...
                jr[r['cssnid']]['account']['statement'][time.strftime("%d/%m/%Y %H:%M:%S")]={"operation":"Withdraw","opamount":str(w),'remainingamount':str(rm-w)}
                fw=open(r"C:\Users\PHANEENDRA S\Desktop\tcs_proj\json\customer_details.json",'w')
                json.dump(jr,fw,indent=2)
                fw.close()
...
def store_deposit():
    ...
    if cid in jr.keys():
        if aid==jr[r['cssnid']]['account']['accountid']:
            ...
            jr[r['cssnid']]['account']['statement'][time.strftime("%d/%m/%Y %H:%M:%S")]={"operation":"Deposit","opamount":str(dm),'remainingamount':str(rm+dm)}
            fw=open(r"C:\Users\PHANEENDRA S\Desktop\tcs_proj\json\customer_details.json",'w')
            json.dump(jr,fw,indent=2)
            fw.close()
    ...
    if fcid in jr.keys():
        if faid==jr[fcid]['account']['accountid']:
            if rm>=tm:
                ...
                jr[fcid]['account']['statement'][time.strftime("%d/%m/%Y %H:%M:%S")]={"operation":"Tranfer"+tcid+taid,"opamount":str(tm),'remainingamount':str(rm-tm)}
                fw=open(r"C:\Users\PHANEENDRA S\Desktop\tcs_proj\json\customer_details.json",'w')
                json.dump(jr,fw,indent=2)
                fw.close()
    ...
    if cid in jr.keys():
        if aid==jr[cid]['account']['accountid']:
            ...
            if selector=="num":
                try:
                    n=int(r["numtext"])
                    li=[[i,j] for i,j in jr[cid]['account']['statement'].items()]
                    if n>len(li):
                        n=len(li)
